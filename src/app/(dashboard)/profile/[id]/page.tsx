...
export default function PeerProfilePage({
  ...
  const { users, projects, connections, toggleConnect, likeProject } = useApp();
  ...
  const connectionStatus = connections[user.id] || "none";

  // Selected project helper
  const selectedProject = projects.find(p => p.id === selectedProjectId);

  // Connections Count Helper
  const displayConnectionsCount = connectionStatus === "connected" 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-card-border bg-[#09090b]/40 overflow-hidden relative shadow-lg">
        {/* Cover image */}
        <div className="h-40 sm:h-56 bg-cover bg-center relative" style={{ backgroundImage: `url('${user.banner}')` }}>
        ...
        <div className="p-6 pt-0 relative flex flex-col sm:flex-row items-center sm:items-end justify-between gap-6 mt-[-40px] sm:mt-[-50px]">
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
            ...
            <div className="mb-2 space-y-1">
              ...
              <span className="text-xs font-semibold text-primary block">{user.role}</span>
          ...
          {/* Action Trigger - Connect Button */}
          <button
            onClick={() => toggleConnect(user.id)}
            className={cn(
              "mb-2 flex items-center justify-center gap-1.5 px-6 py-2.5 rounded-xl text-xs font-bold transition-all border shadow-md",
              connectionStatus === "connected"
                ? "bg-primary/10 border-primary/20 text-primary hover:bg-red-500/10 hover:border-red-500/20 hover:text-red-400 group"
                : connectionStatus === "pending_outgoing"
                ? "bg-transparent border-card-border text-muted-foreground hover:bg-card-border/40 hover:text-white"
        ...
        <div className="px-6 pb-6 text-center sm:text-left">
          ...
          <div className="flex items-center justify-center sm:justify-start gap-3 mt-4 text-muted-foreground">
            {user.socials.website && (
              <a href={user.socials.website} target="_blank" rel="noreferrer" className="hover:text-white transition-colors">
                <Globe className="h-4.5 w-4.5" />
              </a>
            )}
            {user.socials.behance && (
              <a href={user.socials.behance} target="_blank" rel="noreferrer" className="hover:text-white transition-colors">
                <Dribbble className="h-4.5 w-4.5" />
              </a>
            )}
            {user.socials.linkedin && (
              <a href={user.socials.linkedin} target="_blank" rel="noreferrer" className="hover:text-white transition-colors">
              ...
              userProjects.length === 0 ? (
                <div className="p-12 text-center border border-dashed border-card-border rounded-2xl bg-card/10">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    This user hasn&apos;t shared any portfolio pieces yet.
                  </p>
                </div>
              ) : (
            ...
            {activeTab === "experience" && (
              user.experience.length === 0 ? (
                <div className="p-12 text-center border border-dashed border-card-border rounded-2xl bg-card/10">
                  ...
                  <h3 className="text-sm font-bold text-white">No experience listed</h3>
                </div>
              ) : (
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-primary flex items-center gap-1">
                    <Sparkles className="h-3 w-3" /> Creative Journey
                  </span>
          ...
          <div className="p-5 rounded-2xl bg-card/60 border border-card-border shadow-md space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-white">{user.portfolioViews} views</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-medium text-muted-foreground">Connections Network</span>
                <span className="text-xs font-bold text-white">{displayConnectionsCount} members</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col text-left leading-none">
                  ...
                  <span className="text-[9px] text-muted-foreground mt-1">
                    Published by <strong className="text-white font-medium">{user.name}</strong> • {selectedProject.category}
                  </span>
                </div>
              </div>
